...
# Убираем жесткую привязку только к состоянию, либо делаем универсально,
# чтобы бот ловил номера даже если пользователь не нажимал /start в группе
@dp.message(F.text)
async def process_phone(message: types.Message, state: FSMContext):
    ...
    if match:
        ...
        # Если это просто какой-то левый текст или чужая команда — игнорируем,
        # чтобы бот не спамил ошибкой на каждое сообщение в группе.
        # Но если человек находится в состоянии ожидания номера или прислал что-то похожее:
        current_state = await state.get_state()
        if current_state == BotStates.waiting_for_phone.state or len(text) >= 9:
            await message.answer(
